class Solution {
    ...
    bool solve(int r, int c, int m, int n, vector<vector<char>>& board, string word, int k) {
        ...
        // need to rekeep the original character at it's original place because if this path marked false maybe some other place will mark it true !!!
        // for that oiginal look of 2D arrat should remain same !!!!
        board[r][c]=ch;
        return false;
    }
    ...
    bool exist(vector<vector<char>>& board, string word) {
        ...
        for (int i = 0; i < board.size(); i++) {
            for (int j = 0; j < board[0].size(); j++) {
                if (board[i][j] == word[0] && solve(i, j, m, n, board, word, 1)==1) {
                    return 1;
                }
            }
        }
