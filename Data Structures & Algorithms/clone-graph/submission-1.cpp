...
public:
    Node* cloneGraph(Node* node) {
        ...
        // pushing old nodes in queue because traversal will happen on old nodes only !!!
        queue<Node*> q;
        q.push(node);
        ...
        while(!q.empty()){
            ...
            for(auto it:curr->neighbors){
                ...
                // insert the neighbors of new nodes in one traversal only !!!
                // either neighbor pehel se present hoga to naya node nhi banana padega but agar 
                // neighbor pehel se present nhi hai to upar we created it !!
