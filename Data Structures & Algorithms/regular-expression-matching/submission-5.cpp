...
public:
    bool isMatch(string s, string p) {
        ...
        // need to handle the case for i=0; 
        for(int j=1;j<=n2;j++){
            if(j>=2 && p[j-1]=='*'){
                dp[0][j]=dp[0][j-2];
            }
        ...
        for(int i=1;i<=n1;i++){
            for(int j=1;j<=n2;j++){
                // ya to . ho ya to equal element !!
                if(s[i-1]==p[j-1] || p[j-1]=='.'){
                    dp[i][j]=dp[i-1][j-1];
                }
                if(p[j-1]=='*'){
                    // zero occurence is always allowed !!!
                    dp[i][j]=dp[i][j-2];
                    if(s[i-1]==p[j-2] || p[j-2]=='.'){
                        // previous element of p is matching with s in presence of either equality or dot !!!!
